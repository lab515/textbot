...
public class Evaluator {
    ...
    public String evaluate2(OP p, Map variables,Object uo) {
        if (p == null)
            return setErr("emtpy expression!");
        err = null;
        Oper c = null;
        ...
        while (p != null) {
            if (p.Flag == OP.TOKEN_VAR) {
                if (variables == null || (ov = variables.get(p.TkValue)) == null)
                    accessVariable(p.TkValue, c = new Oper("", false, c, p.TkValue),uo);
                else
                ...
                switch (p.Flag) {
                    ...
                    case OP.TOKEN_CALL:
                        ...
                        if (v2.Val == null)
                            v2.Val = "";
                        break;
                    case OP.TOKEN_MEMBER:
                    case OP.TOKEN_MEMBER3:
                        v2.RefVar = v1.RefVar + "." + (v2.RefVar == null ? v2.Val : v2.RefVar);
                        // check the vaues
                        if (variables == null || (ov = variables.get(v2.RefVar)) == null) {
                            v2.Val = "";
                            accessVariable(v2.RefVar, v2,uo);
                        } else
                            v2.Val = ov.toString();
                        v2.isNum = false;
                        break;
